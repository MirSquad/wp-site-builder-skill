...
### Self-Hosting Fonts (preferred over Google Fonts CDN)

Self-host fonts by default instead of loading from Google Fonts' CDN — it removes a third-party request entirely and gives full control over subsetting.

- Fetch the actual CSS from `fonts.googleapis.com/css2?family=...` with a modern browser user-agent to get the real `.woff2` URLs (Google splits by unicode-range per subset — preserve that structure, just point `src` at local files).
- Trim to the subsets actually needed (usually `latin` + `latin-ext` for English-language sites — drop Cyrillic/Vietnamese/Greek unless genuinely needed). This typically cuts the self-hosted payload roughly in half.
- Preload only the 1-2 font files needed for above-the-fold text (body copy + primary heading).

If self-hosting isn't feasible on a given project (tight timeline, a font license that requires CDN delivery), fall back to the async-loading pattern below — it's still far better than a blocking `@import`.

### Google Fonts (fallback — when self-hosting isn't feasible)
...
Small, low-risk CSS additions worth including by default on every build:

- `scrollbar-gutter: stable` on `html` — prevents layout shift when a page's content height crosses the scrollbar-appearance threshold.
- Prefer `dvh`/`svh` over `vh` for any full-viewport-height layout (`min-height: 100vh`) — fixes the mobile Safari address-bar jump. Always pair with a plain `vh` fallback declared first (unsupported browsers ignore the invalid `dvh` line and keep the `vh` one).
- `text-wrap: balance` on headings, `text-wrap: pretty` on paragraphs — pure progressive enhancement, safe to apply globally, unsupported browsers just ignore it.
...
- `input`, `textarea`, and `select` need `font-size: 16px` or higher at mobile breakpoints — anything smaller triggers iOS Safari's auto-zoom on focus. If a third-party embed (payment form, newsletter signup) ships its own smaller font via injected CSS, override it with a scoped `!important` rule targeting its specific selector.
...
## Favicons, App Icons, and Web App Manifest

Add this complete set to every build by default — same tier as a skip link or a viewport meta tag. A partial set is a common, easy-to-miss gap since nothing visually breaks in normal browsing when a piece is missing.

**The five parts:**
1. `favicon.svg` — vector, linked first: `<link rel="icon" href=".../favicon.svg" type="image/svg+xml">`. Modern browsers prefer it; older ones fall through to the PNG links below.
2. `favicon-32.png` / `favicon-192.png` — PNG fallbacks via `<link rel="icon" sizes="...">`.
3. `apple-touch-icon.png` (180×180) — via `<link rel="apple-touch-icon">`.
4. `favicon.ico` at the literal site root — see the nginx gotcha below. This is the one that silently 404s forever if skipped.
5. Maskable icon(s) for the web app manifest — see below.

### Critical gotcha: `/favicon.ico` can bypass WordPress entirely

Most managed WordPress hosts ship an nginx config with a block like:
```nginx
...
```
This checks the docroot for a literal file and returns 404 before falling through to `index.php` — no WordPress hook (`template_redirect`, `init`, anything) can intercept it, at any priority, because the request never reaches PHP. **Do not build a PHP-based "serve this virtual file" solution for `/favicon.ico`** — it cannot work on hosts configured this way, and there's no way to detect the config from inside WordPress.

**How to diagnose this vs. a real gap:** compare the response body, not just the status code. Fetch `/robots.txt` and `/favicon.ico` — if `robots.txt` shows WordPress/Yoast's actual generated content (proving PHP executed) while `favicon.ico` returns a bare `<center>nginx</center>` or similar server-default 404 page with zero WordPress fingerprint, that confirms the request never hit PHP. A WordPress-generated 404 page (theme's `404.php` template) means it did reach PHP and the fix path is different — the file just doesn't exist yet, not a server-level block.

**The only fix:** get a physical `favicon.ico` file placed at the true docroot (same directory as `wp-config.php`) — via SFTP, hosting file manager, or whatever direct file access the host provides. Generate the file locally (e.g. Pillow: `Image.open(src).save('favicon.ico', sizes=[(16,16),(32,32),(48,48)])`) and hand it to the client for upload if you don't have docroot access yourself.

### Web app manifest

- `site.webmanifest` with `name`, `short_name`, `start_url`, `display`, `theme_color`, `background_color`, and an `icons` array covering both `"purpose": "any"` and `"purpose": "maskable"` entries (192px and 512px each).
- **Maskable icons need real padding, not just a resize.** OS icon masks (circle, squircle, etc.) crop to a centered ~80%-diameter safe zone. Take the existing icon, flatten it onto an opaque full-bleed square (no transparent corners — OS masks assume the square is fully painted), then scale the icon content down to roughly 60–65% and center it, so nothing meaningful sits outside the safe zone.
- **Use absolute URLs** for the icon `src` values inside the manifest, not paths relative to the manifest's own location. This makes the exact same manifest file work correctly whether it's served from the theme directory or copied to the site root — no need to maintain two versions with different relative paths.
- Link it via `<link rel="manifest" href="...">`.
- Also place a copy at the conventional root paths (`/site.webmanifest` and `/manifest.json`) if the site is likely to be audited by tools that check those paths by convention rather than reading the actual `<link>` tag. Unlike `favicon.ico`, this usually isn't an nginx block — check the response body the same way (a WordPress 404 template means it's just missing, not blocked) before assuming a workaround is needed.
